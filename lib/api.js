/**
 * NEXT.JS CONCEPT: lib/ folder + environment variables
 *
 * lib/ is a convention (not enforced) for shared utilities.
 *
 * Environment variables in Next.js:
 *   - Server-only:         process.env.MY_SECRET       (in .env.local)
 *   - Exposed to browser:  process.env.NEXT_PUBLIC_*   (MUST have the prefix)
 *
 * In Vite you'd use import.meta.env.VITE_*
 * In Next.js you use process.env.NEXT_PUBLIC_*
 *
...
export async function getLinks() {
  try {
    const res = await fetch(`${BASE}/links`, { cache: "no-store" });
